		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckComputeV2KeypairExists("orangecloud_compute_keypair_v2.kp_1", &keypair),
				),
			},
		},
	})
...
func testAccCheckComputeV2KeypairDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "orangecloud_compute_keypair_v2" {
			continue
		}
...
resource "orangecloud_compute_keypair_v2" "kp_1" {
  name = "kp_1"
