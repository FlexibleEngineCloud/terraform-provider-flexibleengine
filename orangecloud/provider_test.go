...
// Steps for configuring OrangeCloud with SSL validation are here:
...
func TestAccProvider_caCertFile(t *testing.T) {
	if os.Getenv("TF_ACC") == "" || os.Getenv("OS_SSL_TESTS") == "" {
		t.Skip("TF_ACC or OS_SSL_TESTS not set, skipping OrangeCloud SSL test.")
	}
	if os.Getenv("OS_CACERT") == "" {
		t.Skip("OS_CACERT is not set; skipping OrangeCloud CA test.")
	}
	...
	if err != nil {
		t.Fatalf("Unexpected err when specifying OrangeCloud CA by file: %s", err)
	}
}
...
func TestAccProvider_caCertString(t *testing.T) {
	...
	err = p.Configure(terraform.NewResourceConfig(rawConfig))
	if err != nil {
		t.Fatalf("Unexpected err when specifying OrangeCloud CA by string: %s", err)
	}
}
...
func TestAccProvider_clientCertFile(t *testing.T) {
	...
	if os.Getenv("OS_CERT") == "" || os.Getenv("OS_KEY") == "" {
		t.Skip("OS_CERT or OS_KEY is not set; skipping OrangeCloud client SSL auth test.")
	}
	...
	err = p.Configure(terraform.NewResourceConfig(rawConfig))
	if err != nil {
		t.Fatalf("Unexpected err when specifying OrangeCloud Client keypair by file: %s", err)
	}
}
...
func TestAccProvider_clientCertString(t *testing.T) {
	...
	err = p.Configure(terraform.NewResourceConfig(rawConfig))
	if err != nil {
		t.Fatalf("Unexpected err when specifying OrangeCloud Client keypair by contents: %s", err)
	}
}
