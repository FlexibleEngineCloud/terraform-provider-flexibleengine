...
func TestAccCertificateV2DataSource_basic(t *testing.T) {
...
func testAccCheckCertificateV2DataSourceID(n string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		rs, ok := s.RootModule().Resources[n]
		if !ok {
			return fmt.Errorf("Can't find certificate data source: %s", n)
		}

		if rs.Primary.ID == "" {
			return fmt.Errorf("Certificate data source ID not set")
		}
