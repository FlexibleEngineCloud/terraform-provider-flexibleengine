		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(
						"flexibleengine_dds_instance_v3.instance", "name", "dds_instance"),
				),
			},
		},
	})
...
resource "flexibleengine_networking_secgroup_v2" "secgrp_dds" {
  name        = "secgrp-dds-instance"
  description = "DDS Security Group"
}
...
resource "flexibleengine_dds_instance_v3" "instance" {
  ...
  vpc_id = "%s"
  subnet_id = "%s"
  security_group_id = "${flexibleengine_networking_secgroup_v2.secgrp_dds.id}"
  password = "Test@123"
  mode = "Sharding"
  flavor {
    type = "mongos"
  ...
  flavor {
    ...
    storage = "ULTRAHIGH"
    size = 20
    spec_code = "dds.c3.medium.4.config"
  }
  backup_strategy {
