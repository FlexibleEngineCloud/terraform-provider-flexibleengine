...
func resourceNatDnatRuleV2() *schema.Resource {
	return &schema.Resource{
		...
		Update: resourceNatDnatRuleUpdate,
...
func resourceNatDnatRuleCreate(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sdkClient(GetRegion(d, config), "nat")
	if err != nil {
	...
	id, err := navigateValue(r.Body, []string{"dnat", "id"}, nil)
	if err != nil {
...
func resourceNatDnatRuleRead(d *schema.ResourceData, meta interface{}) error {
	...
	url, err := replaceVars(d, "dnat_ruels/{id}", nil)
	if err != nil {
		return err
	}
	url = client.ServiceURL(url)
	...
	adminStateUPProp, err := navigateValue(res, []string{"read", "DnatRule", "admin_state_up"}, nil)
	if err != nil {
		return fmt.Errorf("Error reading Dnat:admin_state_up, err: %s", err)
	}
	if err = d.Set("admin_state_up", adminStateUPProp); err != nil {
		return fmt.Errorf("Error setting Dnat:admin_state_up, err: %s", err)
	}

	createdATProp, err := navigateValue(res, []string{"read", "DnatRule", "created_at"}, nil)
	if err != nil {
	...
	if !ok {
		floatingIPIDProp, err = navigateValue(res, []string{"read", "DnatRule", "floating_ip_id"}, nil)
		if err != nil {
	...
	if !ok {
		internalServicePortProp, err = navigateValue(res, []string{"read", "DnatRule", "internal_service_port"}, nil)
		if err != nil {
	...
	if !ok {
		natGatewayIDProp, err = navigateValue(res, []string{"read", "DnatRule", "nat_gateway_id"}, nil)
		if err != nil {
	...
	if !ok {
		portIDProp, err = navigateValue(res, []string{"read", "DnatRule", "port_id"}, nil)
		if err != nil {
	...
	if !ok {
		privateIPProp, err = navigateValue(res, []string{"read", "DnatRule", "private_ip"}, nil)
		if err != nil {
	...
	if !ok {
		protocolProp, err = navigateValue(res, []string{"read", "DnatRule", "protocol"}, nil)
		if err != nil {
	...
	statusProp, err := navigateValue(res, []string{"read", "DnatRule", "status"}, nil)
	if err != nil {
	...
	tenantIDProp, err := navigateValue(res, []string{"read", "DnatRule", "tenant_id"}, nil)
	if err != nil {
...
func resourceNatDnatRuleUpdate(d *schema.ResourceData, meta interface{}) error {
	config := meta.(*Config)
	client, err := config.sdkClient(GetRegion(d, config), "nat")
	if err != nil {
		return fmt.Errorf("Error creating sdk client, err=%s", err)
	...
	log.Printf("[DEBUG] Updating Dnat %q: %#v", d.Id(), params)
	...
	r := golangsdk.Result{}
	_, r.Err = client.Put(url, &params, &r.Body, &golangsdk.RequestOpts{
		OkCodes: successHTTPCodes,
	})
	if r.Err != nil {
		return fmt.Errorf("Error updating Dnat %q: %s", d.Id(), r.Err)
	}
