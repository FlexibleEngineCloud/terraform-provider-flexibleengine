...
func resourceComputeInterfaceAttachV2Create(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] flexibleengine_compute_interface_attach_v2 attach options: %#v", attachOpts)
	...
	if _, err = stateConf.WaitForState(); err != nil {
		return fmt.Errorf("Error creating flexibleengine_compute_interface_attach_v2 %s: %s", instanceId, err)
	}

	// Use the instance ID and attachment ID as the resource ID.
	id := fmt.Sprintf("%s/%s", instanceId, attachment.PortID)

	log.Printf("[DEBUG] Created flexibleengine_compute_interface_attach_v2 %s: %#v", id, attachment)
...
func resourceComputeInterfaceAttachV2Read(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return CheckDeleted(d, err, "Error retrieving flexibleengine_compute_interface_attach_v2")
	}

	log.Printf("[DEBUG] Retrieved flexibleengine_compute_interface_attach_v2 %s: %#v", d.Id(), attachment)
...
func resourceComputeInterfaceAttachV2Delete(d *schema.ResourceData, meta interface{}) error {
	...
	if _, err = stateConf.WaitForState(); err != nil {
		return fmt.Errorf("Error detaching flexibleengine_compute_interface_attach_v2 %s: %s", d.Id(), err)
	}
