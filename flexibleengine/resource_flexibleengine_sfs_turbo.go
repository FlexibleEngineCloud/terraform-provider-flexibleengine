...
func resourceSFSTurboCreate(d *schema.ResourceData, meta interface{}) error {
	...
	if err != nil {
		return fmt.Errorf("Error creating FlexibleEngine SFS Turbo: %s", err)
	}
	d.SetId(create.ID)
...
func resourceSFSTurboUpdate(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("size") {
		...
		if old.(int) > newsize.(int) {
			return fmt.Errorf("Shrinking FlexibleEngine SFS Turbo size is not supported")
		}
		...
		if err != nil {
			return fmt.Errorf("Error deleting FlexibleEngine SFS Turbo: %s", err)
		}
	}
