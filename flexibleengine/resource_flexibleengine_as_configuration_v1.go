	...
	for _, v := range diskMeta {
		...
		if diskType == "SYS" {
			if size < 1 || size > 32768 {
				return diskOptsList, fmt.Errorf("For system disk size should be [1, 32768]")
			}
		}
		if diskType == "DATA" {
...
func getASGroupsByConfiguration(asClient *gophercloud.ServiceClientExtension, configurationID string) ([]groups.Group, error) {
	var gs []groups.Group
