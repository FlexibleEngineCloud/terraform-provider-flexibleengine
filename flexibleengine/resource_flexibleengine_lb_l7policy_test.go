		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLBV2L7PolicyExists("flexibleengine_lb_l7policy.l7policy_1", &l7Policy),
					resource.TestCheckResourceAttr(
						"flexibleengine_lb_l7policy.l7policy_1", "name", "test"),
					resource.TestCheckResourceAttr(
						"flexibleengine_lb_l7policy.l7policy_1", "description", "test description"),
					resource.TestCheckResourceAttr(
						"flexibleengine_lb_l7policy.l7policy_1", "action", "REDIRECT_TO_POOL"),
					resource.TestCheckResourceAttr(
						"flexibleengine_lb_l7policy.l7policy_1", "position", "1"),
					resource.TestMatchResourceAttr(
						"flexibleengine_lb_l7policy.l7policy_1", "listener_id",
...
func testAccCheckLBV2L7PolicyDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "flexibleengine_lb_l7policy" {
			continue
		}
