...
func TestAccObsBucketPolicy_basic(t *testing.T) {
	...
	policyName := "flexibleengine_obs_bucket_policy.policy"
...
func TestAccObsBucketPolicy_s3(t *testing.T) {
	name := fmt.Sprintf("tf-test-bucket-%d", acctest.RandInt())
	obsName := "flexibleengine_obs_bucket.bucket"
	policyName := "flexibleengine_obs_bucket_policy.s3_policy"
...
func testAccCheckObsBucketHasPolicy(n string, expectedPolicyText string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if format == "obs" {
			obsClient, err = conf.ObjectStorageClientWithSignature(OS_REGION_NAME)
		} else {
			obsClient, err = conf.ObjectStorageClient(OS_REGION_NAME)
		}
		if err != nil {
...
func testAccOBSPolicyImportStateIDFunc() resource.ImportStateIdFunc {
	return func(s *terraform.State) (string, error) {
		policyRes, ok := s.RootModule().Resources["flexibleengine_obs_bucket_policy.s3_policy"]
		if !ok {
