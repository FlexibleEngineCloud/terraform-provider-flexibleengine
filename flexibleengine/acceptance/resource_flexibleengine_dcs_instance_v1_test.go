		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrPair(resourceName, "flavor",
						"data.flexibleengine_dcs_flavors.test", "flavors.0.name"),
...
func TestAccDcsInstances_prePaid(t *testing.T) {
	...
	resourceName := "flexibleengine_dcs_instance_v1.test"
...
data "flexibleengine_dcs_backup" "test" {
  cache_mode     = "ha"
  capacity       = 0.125
  engine_version = "5.0"
}
...
resource "flexibleengine_dcs_instance_v1" "instance_1" {
		...
		availability_zones = [data.flexibleengine_availability_zones.test.names[0]]
flavor             = data.flexibleengine_dcs_flavors.test.flavors[0].name
...
resource "flexibleengine_dcs_instance_v1" "test" {
  vpc_id             = data.flexibleengine_vpc_v1.test.id
  subnet_id          = data.flexibleengine_vpc_subnet_v1.test.id
  availability_zones = try(slice(data.flexibleengine_availability_zones.test.names, 0, 1), [])
  name               = "%s"
  engine             = "Redis"
  engine_version     = "5.0"
  flavor             = data.flexibleengine_dcs_flavors.test.flavors[0].name
