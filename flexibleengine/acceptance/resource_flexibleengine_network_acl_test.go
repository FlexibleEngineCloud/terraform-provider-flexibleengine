...
func TestAccNetworkACL_basic(t *testing.T) {
	...
	resourceKey := "flexibleengine_network_acl.fw_1"
	...
	resource.ParallelTest(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNetworkACLExists("flexibleengine_network_acl.fw_1", &fwGroup),
...
func testAccCheckNetworkACLDestroy(s *terraform.State) error {
	...
	if err != nil {
		return fmtp.Errorf("Error creating FlexibleEngine fw client: %s", err)
	}
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "flexibleengine_network_acl" {
			continue
		}
...
resource "flexibleengine_network_acl" "fw_1" {
  ...
  inbound_rules = [flexibleengine_network_acl_rule.rule_1.id, flexibleengine_network_acl_rule.rule_2.id]
  subnets       = [flexibleengine_vpc_subnet_v1.subnet_1.id, flexibleengine_vpc_subnet_v1.subnet_2.id]
}
`, testAccNetworkACLRules(name), name)
}
