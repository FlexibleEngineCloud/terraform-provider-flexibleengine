...
func TestAccCSBSBackupPolicyV1DataSource_basic(t *testing.T) {
	dataName := "data.flexibleengine_csbs_backup_policy_v1.csbs_policy"
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckCSBSBackupPolicyV1DataSourceID(dataName),
					resource.TestCheckResourceAttr(dataName, "name", "backup-policy"),
					resource.TestCheckResourceAttr(dataName, "status", "suspended"),
				),
			},
		},
	})
